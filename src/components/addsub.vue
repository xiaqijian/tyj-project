<template>
  ...
  <el-form-item label="选择用户">
    <el-select v-model="form.region" placeholder="选择用户">
      <el-option v-for="item in uid" :key="item" :label="item.uname" :value="item.uid"></el-option>
    </el-select>
  </el-form-item>
...
export default {
  ...
  methods: {
      onSubmit () {
        ...
        //     console.log(error);
        // });
        let sname = that.form.name;
        let uid = that.form.region;
        that.$axios({
            method: 'post',
            url: 'api/tianyujiat/sub!addSub.action',
            data: qs.stringify({
                'subject.sname': sname,
               'subject.uid':uid
            }),
